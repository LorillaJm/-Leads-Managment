...
export function SalesTeamTable({ data, totalCount }: SalesTeamTableProps) {
  ...
  return (
    ...
    >
      ...
      <div className="px-2.5 py-1 bg-gray-50 border-b border-gray-200 text-right">
        <span className="text-[10px] text-gray-600">Count: {totalCount}</span>
      </div>
        ...
        <div className="overflow-x-auto overflow-y-auto flex-1">
          <table className="w-full">
            <thead className="bg-blue-600 text-white sticky top-0 z-10">
              <tr>
                <th className="px-1.5 py-1 text-left text-[9px] font-semibold whitespace-nowrap">
                  <button className="flex items-center gap-0.5 hover:text-blue-100">
                    Sales Consultant
                    <ChevronDown className="w-2.5 h-2.5" />
                  </button>
                </th>
                <th className="px-1.5 py-1 text-center text-[9px] font-semibold whitespace-nowrap">
                  <button
            ...
            <tbody className="divide-y divide-gray-200">
              {sortedData.map((consultant, index) => (
                ...
                >
                  <td className="px-1.5 py-1 text-[10px] text-gray-900">
                    {consultant.name}
                  </td>
                  <td className="px-1.5 py-1 text-[10px] text-center text-gray-900">
                    {consultant.leads}
                  </td>
                  <td className="px-1.5 py-1 text-[10px] text-center text-gray-900">
